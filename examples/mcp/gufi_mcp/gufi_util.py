...
class GufiMcpSettings:
    ...
    def load(cls, env_file: Path | None = None) -> "GufiMcpSettings":
        ...
        if gufi_vt_lib is None:
            default_vt = server_prefix / "lib" / "gufi_vt.so"
            if default_vt.is_file():
                gufi_vt_lib = default_vt.resolve()

        event_log_path: Path | None
        log_raw = os.getenv("GUFI_MCP_EVENT_LOG", "").strip().strip("'\"")
        if log_raw.lower() in {"0", "false", "off", "none"}:
            event_log_path = None
        elif log_raw:
            event_log_path = Path(log_raw)
            if not event_log_path.is_absolute():
                event_log_path = (_PACKAGE_DIR / event_log_path).resolve()
        else:
            event_log_path = (_PACKAGE_DIR / "logs" / "gufi_mcp_events.jsonl").resolve()
...
class TreesummaryStatus(TypedDict):
    index_name: str
    target_path: str
    db_path: str | None
    treesummary_available: bool
    build_tool_configured: bool
    warnings: list[str]
    execution_time_ms: float
...
def inspect_treesummary_status(idx_path: str) -> TreesummaryStatus:
    """
    Lightweight check: does treesummary exist in the target index shard db.db?

    idx_path is an index name (e.g. 'notes') or 'index_name/sub/path' for a subtree.
    Does not run gufi_query or build treesummary — only inspects sqlite_master.
    """
    ...
    try:
        ...
        return TreesummaryStatus(
            index_name=Path(idx_path).parts[0] if Path(idx_path).parts else idx_path,
            target_path="",
            db_path=None,
            treesummary_available=False,
            build_tool_configured=bool(os.getenv("GUFI_TREESUMMARY_EXE")),
            warnings=[str(exc)],
    ...
    db_path = start_dir / "db.db"
    if not db_path.is_file():
        warnings.append(f"No db.db at '{db_path}'")
        available = False
    else:
        available = check_treesummary(start_dir)
        if not available:
            warnings.append(
                "treesummary table not present in db.db; -T / tree_sql pruning is unavailable "
                "unless treesummary is built (see GUFI_TREESUMMARY_EXE)."
            )
    ...
    return TreesummaryStatus(
        ...
        treesummary_available=available,
        build_tool_configured=bool(os.getenv("GUFI_TREESUMMARY_EXE")),
        warnings=warnings,
        execution_time_ms=round((time.perf_counter() - t_start) * 1000, 2),
    )
